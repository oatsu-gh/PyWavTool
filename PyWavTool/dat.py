...
class Dat:
    ...
    def read(self, samplewidth: int):
        ...
        if samplewidth == 8:
            ...
            for i in range(int(len(tmp) / samplewidth * 8)):
                self._data.append(
                    int.from_bytes(
                        tmp[i * int(samplewidth / 8) : (i + 1) * int(samplewidth / 8)],
                        "little",
                        signed=True,
                    )
                )
            ...
            for i in range(self._data.shape[0]):
                self._data[i] = int.from_bytes(
                    tmp[i * 3 : (i + 1) * 3], "little", signed=True
                )
        elif samplewidth == 32:
    ...
    def addframe(
        self, data: np.ndarray, ove: float, samplewidth: int, framerate: int
    ) -> int:
        """
    ...
    def addframeAndWrite(
        ...
        with open(output, "r+b") as fw:
            ...
            if samplewidth == 24:
                ...
                byte_data = b"".join(x.to_bytes(3, "little", signed=True) for x in data)
            else:
                if samplewidth == 8:
                    byte_data = data.astype("int8").tobytes()
                elif samplewidth == 16:
                    byte_data = data.astype("int16").tobytes()
                elif samplewidth == 32:
                    byte_data = data.astype("int32").tobytes()
            fw.write(byte_data)
