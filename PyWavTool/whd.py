...
class Whd:
    ...
    def _read(self, whdfile: str):
        ...
        with open(whdfile, "rb") as fr:
            ...
            self._nframes = (
                int.from_bytes(data[40:44], "little") / self._samplewidth * 8
            )
    ...
    def write(self, output: str):
        ...
        with open(output + ".whd", "wb") as fw:
            fw.write(b"RIFF")
            fw.write(
                int(self._nframes * self._samplewidth / 8 + 36).to_bytes(4, "little")
            )
            fw.write(b"WAVE")
            fw.write(b"fmt ")
            ...
            fw.write(
                int(self._framerate * self._samplewidth / 8 * self._nchannels).to_bytes(
                    4, "little"
                )
            )
